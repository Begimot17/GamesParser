    ...
    try:
        async with parser_class() as parser:
            ...
            for i, post in enumerate(posts, 1):
                ...
                assert isinstance(post.date, datetime) or post.date is None, f"Post {i} has invalid date"
                assert isinstance(post.metadata.rating, str), f"Post {i} has invalid rating"
                assert isinstance(post.metadata.store_links, dict), f"Post {i} has invalid store links"
                assert isinstance(post.metadata.images, list), f"Post {i} has invalid images"
                
                # Проверяем URL изображений
                for img_url in post.metadata.images:
                    assert img_url.startswith('http'), f"Invalid image URL in post {i}: {img_url}"
                
                # Проверяем URL магазинов
                for store, url in post.metadata.store_links.items():
                    assert url.startswith('http'), f"Invalid store URL in post {i}: {url}"
