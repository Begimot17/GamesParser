...
def test_parse_article(parser, expected_data):
    ...
    for html_file in html_dir.glob("*.html"):
        ...
        assert set(post.images) == set(expected["images"]), (
            f"Images mismatch in {html_file}"
        )
        assert post.stores == expected["stores"], f"Stores mismatch in {html_file}"

        # Compare metadata
        assert post.metadata.author == expected["metadata"]["author"], (
            f"Author mismatch in {html_file}"
        )
        assert post.metadata.date == expected["metadata"]["date"], (
            f"Date mismatch in {html_file}"
        )
        assert post.metadata.tags == expected["metadata"]["tags"], (
            f"Tags mismatch in {html_file}"
        )
